...
class Messages{
	...
	public static byte[] CreateSetAvatarPositionRotationMessage(
        ...
        m[2] = movementState;
        int count = 3;
        //multiply by 100 to get better accuracy
        AddIntToByteArray(Convert.ToInt16(x*100), ref m, ref count);
        AddIntToByteArray(Convert.ToInt16(z*100), ref m, ref count);
        AddIntToByteArray(Convert.ToInt16((r * 255) / 360), ref m, ref count);
	...
	public static void UnmarshallPositionRotationMessage(
        ...
        //divide by 100 to get correct placement
...
public class ServerNetworkScript : MonoBehaviour {
    ...
    //using booleans in case one player disconnects and tries to reconnect we can still add them to the right character
    Boolean floraIsUsed = false;
    Boolean tommyIsUsed = false;
	...
	void AddNewClient(int recHostId, int connectionId, int channelId) {
        ...
        //start with a byte that should not be chosen
        byte avatarForThisConnection = (byte)3; //byte 3 means nothing, it WILL be changed after this if statement

        //this is done in case a player exits and would like to reconnect to the server. This makes it so that the 
        //server doesn't have to be reset everytime a client accidentally disconnects.
        if (!floraIsUsed)
        {
            avatarForThisConnection = Messages.flora;
            floraIsUsed = true;
        }
        else if (!tommyIsUsed)
        {
            avatarForThisConnection = Messages.tommy;
            tommyIsUsed = true;
        }
        else
        {
            throw new Exception(); // this will only be hit if more than 2 clients are connected.
        }

        //assert that avatar is either flora or tommy
        Debug.Assert(avatarForThisConnection == Messages.flora || avatarForThisConnection == Messages.tommy);
        ...
        //tell the player his PID
        byte[] PlayerIDMessage = Messages.CreateSetPlayerIdMessage(playerIdOpen);
        playerIdOpen++;
        byte sendPIDerror;
        //this is an important message and should be sent on a reliable channel
        NetworkTransport.Send(hostId, connectionId, controlChannelId, PlayerIDMessage, PlayerIDMessage.Length, out sendPIDerror);
        Messages.LogNetworkError(sendPIDerror);


        //tell client which avatar they should be using
        byte[] AvatarMessage = Messages.CreateUseAvatarMessage(avatarForThisConnection);
        
        byte SendAvatarError;
        //this is an important message and should be sent on a reliable channel
        NetworkTransport.Send(hostId, connectionId, controlChannelId, AvatarMessage, AvatarMessage.Length, out SendAvatarError);
        Messages.LogNetworkError(SendAvatarError);
    }
	...
	void ForwardMovementMessage(int senderConnectionId, byte[] message, int messageLength) {
        ...
        foreach (KeyValuePair<int, byte> connection in connections)
        {
            ...
            byte error;
            //movement messages are not necessarily game critical and should be sent on the unreliable channel
            //to get a better performance
            NetworkTransport.Send(hostId, connection.Key, dataChannelId, message, messageLength, out error);
		...
		while(messagesAvailable) {
			...
			case NetworkEventType.DisconnectEvent:
				Debug.Log(string.Format("Disconnection received from host {0}, connection {1}, channel {2}",
					recHostId, connectionId, channelId));
                    byte disconnectedAvatar;
                    connections.TryGetValue(connectionId, out disconnectedAvatar);
                    if (disconnectedAvatar == Messages.flora)
                    {
                        floraIsUsed = false;
                    }
                    else if (disconnectedAvatar == Messages.tommy)
                    {
                        tommyIsUsed = false;
                    }
                    Debug.Log(string.Format("{0} is the avatar that has been disconnected.", disconnectedAvatar));
