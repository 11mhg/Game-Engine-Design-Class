...
public class ClientNetworkScript : MonoBehaviour {
	...
	string serverHostIP = "127.0.0.1";	// use this IP address if the server is located on the same computer as the clients
	...
	void InitNetwork() {
		...
		byte error;
		_connectionId = NetworkTransport.Connect(_hostId, serverHostIP, _serverPort, 0, out error);
	...
	void UpdateRemoteAvatar(byte[] message) {
		...
		float x, z, r;
		byte movementState;
        Messages.UnmarshallPositionRotationMessage(message, out avatarId, out x, out z, out r, out movementState);
        //rotation needs to be put back in the right degrees
        r = (r * 360f) / 255.0f;

        Debug.Assert(_localAvatarId != avatarId);
        RemoteAvatarScript _remoteScript = _remoteAvatar.GetComponent<RemoteAvatarScript>();
        _remoteScript.targetMovementState = movementState;
        _remoteScript.targetPosition = new Vector3(x,_remoteAvatar.transform.position.y, z);
        _remoteScript.targetRotation = Quaternion.Euler(0, r, 0);
	}
	...
	void ProcessMessage(byte[] message, int messageLength) {
		...
		if(messageType == Messages.setPlayerId) {
            ...
            byte avatarId = Messages.GetAvatarIdFromMessage(message);
            Debug.LogFormat("Received message: Updating avatar {0} to new positions", avatarId);
		} else {
	...
	IEnumerator SendLocalAvatarPositions() {
		...
		while(true) {
			if(_isConnected && _localAvatar != null) {
                //send message with the location,rotation and movement state of this avatar
                float rotation = (_localAvatar.transform.rotation.eulerAngles.y);
                byte[] message = Messages.CreateSetAvatarPositionRotationMessage(_localAvatarId, _localAvatar.transform.position.x, _localAvatar.transform.position.z, rotation, _localAvatar.GetComponent<LocalAvatarScript>().movementState);
                byte error;
